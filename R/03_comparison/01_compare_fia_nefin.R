...
compare_5y_window <- function(fia_df,
  ...
  # FIXED: Use pooled variance for single-plot hexes
  # Problem: sd() returns NA when n=1, making nefin_se_5y = NA
  # Solution: Compute pooled SD across ALL NEFIN plots, use for single-plot hexes
  
  # Step 1: Compute pooled SD from all NEFIN plots (typical plot-to-plot variability)
  pooled_nefin_sd <- sd(nefin_xy$aglb_Mg_per_ha, na.rm = TRUE)
  cat("  Pooled NEFIN SD (for single-plot hexes):", round(pooled_nefin_sd, 2), "Mg/ha\n")
  
  # Step 2: Compute per-hex statistics
  nefin_5y <- nefin_xy %>%
    group_by(hex_id) %>%
    summarise(
      nefin_mean_5y = mean(aglb_Mg_per_ha, na.rm = TRUE),
      nefin_sd_raw  = sd(aglb_Mg_per_ha, na.rm = TRUE),  # NA when n=1
      nefin_n_5y    = n(),
      .groups = "drop"
    ) %>%
    mutate(
      # Flag: TRUE if SE is based on actual within-hex variance (n>=2)
      # FALSE if using pooled variance (n=1, conservative estimate)
      nefin_se_empirical = !is.na(nefin_sd_raw) & nefin_n_5y >= 2,
      
      # Use pooled SD when within-hex SD can't be computed (n=1)
      nefin_sd_adj = if_else(is.na(nefin_sd_raw) | nefin_n_5y < 2, 
                             pooled_nefin_sd, 
                             nefin_sd_raw),
      # SE = SD / sqrt(n)
      nefin_se_5y = nefin_sd_adj / sqrt(nefin_n_5y)
    ) %>%
    select(-nefin_sd_raw, -nefin_sd_adj)
  
  cat("  NEFIN hexes with valid SE:", sum(!is.na(nefin_5y$nefin_se_5y)), "/", nrow(nefin_5y), "\n")
  cat("    - Empirical SE (n>=2):", sum(nefin_5y$nefin_se_empirical), "hexes\n")
  cat("    - Pooled SE (n=1):    ", sum(!nefin_5y$nefin_se_empirical), "hexes\n")
