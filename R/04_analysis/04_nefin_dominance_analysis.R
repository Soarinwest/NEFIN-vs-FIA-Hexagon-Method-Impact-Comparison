...
# Auto-detect consolidated directory (most recent)
all_dirs <- list.dirs("runs", recursive = FALSE, full.names = TRUE)
consol_dirs <- all_dirs[grepl("consolidated_", all_dirs)]
if (length(consol_dirs) == 0) {
  stop("No consolidated directory found in runs/")
}
CONSOLIDATED_DIR <- sort(consol_dirs, decreasing = TRUE)[1]
cat("Using consolidated directory:", CONSOLIDATED_DIR, "\n")

COMPARISON_DATA <- file.path(CONSOLIDATED_DIR, "fia_nefin_comparison_all_scales.csv")
OUTPUT_DIR <- file.path(CONSOLIDATED_DIR, "dominance_analysis")
